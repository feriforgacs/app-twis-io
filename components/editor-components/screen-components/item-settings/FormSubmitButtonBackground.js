...
export default function FormSubmitButtonBackground() {
	...
	const [background, setBackground] = useState(activeScreenItem.settings.highlightColorButton.background || ""); // this is to handle the background that was picked
	const [backgroundColor, setBackgroundColor] = useState(activeScreenItem.settings.highlightColorButton.backgroundColor || ""); // this is to handle the background color that is displayed
	const [colorPickerVisible, setColorPickerVisible] = useState(false);
	const [backgroundType, setBackgroundType] = useState(activeScreenItem.settings.highlightColorButton.type || "solid");

	/**
	 * Update settings on active item change
	 */
	useEffect(() => {
		setBackground(activeScreenItem.settings.highlightColorButton.background);
		setBackgroundColor(activeScreenItem.settings.highlightColorButton.backgroundColor);
		setBackgroundType(activeScreenItem.settings.highlightColorButton.type || "solid");
	}, [activeScreenItem]);
	...
	return (
		<div className={`${styles.settingsSection} screen-settings`}>
			<label className={`${styles.settingsLabel} screen-settings`}>Form Button Background Color</label>
					...
					{backgroundType === "solid" && (
						<SketchPicker
							...
							onChangeComplete={(color) => {
								...
								setActiveScreenItem({ ...activeScreenItem, settings: { ...activeScreenItem.settings, highlightColorButton: { type: backgroundType, backgroundColor: `rgba(${color.rgb.r}, ${color.rgb.g}, ${color.rgb.b}, ${color.rgb.a})`, background: color.rgb } } });
							}}
						/>
					)}
							...
							onSelect={(background) => {
								updateScreenItem(activeScreen.screenId, activeScreenItem.itemId, {
									settings: {
										...activeScreenItem.settings,
										highlightColorButton: {
											...activeScreenItem.settings.highlightColorButton,
											type: backgroundType,
