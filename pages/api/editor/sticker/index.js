...
export default async function StickerRequestHandler(req, res) {
	await cors(req, res);
	...
	if (page > 1 && keyword !== "") {
		// load more gifs
		try {
			giphyResult = await axios.get("https://api.giphy.com/v1/stickers/search", {
				params: {
		...
		try {
			giphyResult = await axios.get("https://api.giphy.com/v1/stickers/trending", {
				params: {
					api_key: process.env.GIPHY_API_KEY,
					limit: 10,
					offset: 10 * (page - 1),
